...
- 只关注“最近 7 天未上榜，且首次上线未满 180 天（约 6 个月）”的游戏
- 聚合 4 个榜单后去重
- 优先显示排名更高的记录
- 最多显示 10 条
- 游戏名优先显示“英文名 + 中文名”
- 每个游戏的上榜来源单独另起一行展示，便于对齐阅读
- 不同榜单标签使用不同颜色；其中 `收入榜` 最醒目且优先展示，`新秀榜` 次之
...
如果 `ROBLOX_PROJECT_METRICS_DISABLE_SECOND_PROJECT=true`，项目日报会临时跳过第二项目槽位 `ROBLOX_CREATOR_OVERVIEW_URL_2`。这只影响 `roblox_project_daily_metrics`：不抓取第二项目、不写第二项目飞书表、不在 `project_metrics_*.json/csv` 中输出第二项目记录，也不发送第二项目表格链接；`roblox_money` 收入日报仍会继续使用第二项目 overview URL。当前 GitHub Actions workflow 为临时屏蔽 Jail，未配置该变量时按 `true` 注入；恢复第二项目日报时需要在 GitHub Variables 中明确设为 `false`，或把 workflow 默认值改回 `false`。
...
- `ROBLOX_CREATOR_OVERVIEW_URL`
- `ROBLOX_CREATOR_OVERVIEW_URL_2`
- `ROBLOX_PROJECT_METRICS_DISABLE_SECOND_PROJECT`，可选；当前 GitHub Actions 未配置时按 `true` 注入，用于临时屏蔽第二项目槽位；恢复时设为 `false`
- `FEISHU_PROJECT_METRICS_SPREADSHEET_TOKEN`
- `FEISHU_PROJECT_METRICS_SHEET_ID`
- `FEISHU_PROJECT_METRICS_SPREADSHEET_TITLE`
- `FEISHU_PROJECT_METRICS_2_SPREADSHEET_TOKEN`
