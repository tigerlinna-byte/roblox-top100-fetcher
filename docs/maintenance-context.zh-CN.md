...
项目日报的核心字段以 Roblox Creator Dashboard Explore 页面每日粒度为准，不使用 Overview 顶部卡片或 7 日平均口径：

- `AveragePlayTimeMinutesPerDAU` -> `平均在线时长` 和 `平均在线时长同类排名`
- `ForwardD1Retention` -> `次留` 和 `次留同类排名`
- `ForwardD7Retention` -> `7留` 和 `7留同类排名`
- `PayingUsersCVR` -> `付费率` 和 `付费率同类排名`
- `AverageRevenuePerPayingUser` -> `付费用户平均收入` 和 `付费用户平均收入同类排名`
- `RFYQualifiedPTR` -> `QPTR`
- `RFYPlayThroughRate` -> `PTR`
- `RFYDeepEngagementRate` -> `DPTR`

这些字段通过 Roblox Analytics Query Gateway 读取 Daily 序列；同类排名优先从 Daily 返回数据中的 benchmark percentile / `currentPercentile` 提取，对应 Explore URL 中的 `granularity=Daily` 和 `overlays=benchmark` 口径。若 Daily datapoint 暂未返回 percentile，则使用同名 Daily 指标请求 Overview `benchmark-scorecard` 接口作为排名兜底，仅补充排名列，不覆盖 Daily 数值列；兜底请求不使用 `L7Average...` 指标。`付费用户平均收入` 按 Roblox Daily 指标原始数值写入，不自动加美元符号。

当前代码内临时开启了 `PROJECT_METRICS_FORCE_DAILY_CORE_REFRESH = True`，用于修正飞书表格中已经写入的旧口径核心字段。该开关只强制回查并覆盖上述 Daily 核心字段，不影响 `PeakConcurrentPlayers`、`ARPDAU`、五分钟留存、Home Recommendation、性能和崩溃类字段的“只补空”日常逻辑。表格历史数据修正完成后，应将该常量改回 `False` 并提交，恢复日常增量更新。

项目日报的其他补充字段主要直接走 Roblox Analytics Query Gateway：
...
客户端设备内存使用 `ClientMemoryUsageAvg`，沿用 `Platform` breakdown 分别读取 `Tablet`、`Computer` 和 `Phone`。接口原始值按 MB 处理，同一业务日有多个点时先取日平均，再按 `1024 MB = 1 GB` 换算，最多保留两位小数并去掉末尾多余零。飞书中的 `平板内存`、`PC内存`、`手机内存` 均显示为 GB；旧表中的百分比属于已废弃口径，读取时会视为空值并进入字段级历史回填，不能回填实际值的日期保持空白。
...
写表后会先把当前排名列和旧版排名列字体重置为黑色，并清理旧版排名列粗体残留，再把当前排名列数据区设置为加粗，最后对有有效数值的当前排名单元格应用渐变色，避免新增列后旧排名样式残留到普通指标列。
