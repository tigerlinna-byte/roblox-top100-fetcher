...
项目日报的核心 Overview 卡片字段以 Roblox Creator Dashboard Overview 页面展示值为准：

- `Average playtime` -> `平均在线时长` 和 `平均在线时长同类排名`
- `Day 1 retention` -> `次留` 和 `次留同类排名`
- `Day 7 retention` -> `7留` 和 `7留同类排名`
- `Payer conversion rate` -> `付费率` 和 `付费率同类排名`
- `Average revenue per paying user` -> `付费用户平均收入` 和 `付费用户平均收入同类排名`
- `Qualified play through rate` -> `QPTR`

这些字段通过 Roblox Overview scorecard 接口读取 `currentValue` 和 `currentPercentile`，以保证飞书表格与 Overview 页面卡片一致。其中 `付费用户平均收入` 按 Overview 页面 Robux 数值写入，不再按美元格式写入。

当前代码内临时开启了 `PROJECT_METRICS_FORCE_OVERVIEW_REFRESH = True`，用于修正飞书表格中已经写入的旧口径核心 Overview 字段。该开关只强制回查并覆盖上述 Overview 卡片字段，不影响 `PeakConcurrentPlayers`、`ARPDAU`、五分钟留存、Home Recommendation、性能和崩溃类字段的“只补空”日常逻辑。表格历史数据修正完成后，应将该常量改回 `False` 并提交，恢复日常增量更新。
