...
这些字段通过 Roblox Analytics Query Gateway 读取 Daily 序列；同类排名从 Daily 返回数据中的 benchmark percentile / `currentPercentile` 提取，对应 Explore URL 中的 `granularity=Daily` 和 `overlays=benchmark` 口径。`付费用户平均收入` 按 Roblox Daily 指标原始数值写入，不自动加美元符号。
