...
PROJECT_METRICS_5_SPREADSHEET_TOKEN_VAR = "FEISHU_PROJECT_METRICS_5_SPREADSHEET_TOKEN"
PROJECT_METRICS_5_SHEET_ID_VAR = "FEISHU_PROJECT_METRICS_5_SHEET_ID"
...
def resolve_project_metrics_variables(cfg: Config) -> tuple[ProjectMetricsSheetVariables, ...]:
    ...
    third_project_id = _extract_project_id(cfg.roblox_creator_overview_url_3)
    if third_project_id:
        resolved.append(
            ProjectMetricsSheetVariables(
                project_id=third_project_id,
                overview_url=cfg.roblox_creator_overview_url_3,
                spreadsheet_token_variable_name=PROJECT_METRICS_3_SPREADSHEET_TOKEN_VAR,
                sheet_id_variable_name=PROJECT_METRICS_3_SHEET_ID_VAR,
    ...
    fifth_project_id = _extract_project_id(cfg.roblox_creator_overview_url_5)
    if fifth_project_id:
        resolved.append(
            ProjectMetricsSheetVariables(
                project_id=fifth_project_id,
                overview_url=cfg.roblox_creator_overview_url_5,
                spreadsheet_token_variable_name=PROJECT_METRICS_5_SPREADSHEET_TOKEN_VAR,
                sheet_id_variable_name=PROJECT_METRICS_5_SHEET_ID_VAR,
                spreadsheet_token=cfg.feishu_project_metrics_5_spreadsheet_token,
                sheet_id=cfg.feishu_project_metrics_5_sheet_id,
                spreadsheet_title=cfg.feishu_project_metrics_5_spreadsheet_title,
            )
        )
...
def _merge_single_record(rows: list[list[object]], record: ProjectDailyMetricsRecord) -> None:
    ...
    for index, value in enumerate(row_values):
        ...
        if text and (not current_text or can_overwrite):
            current_row[index] = text
            changed_value = True
    if (is_new_row or changed_value) and row_values[-1]:
...
def _extract_shifted_legacy_row_field_values(
    ...
    field_values: dict[str, str] = {
        ...
        "ptr": current_values.get("ptr", ""),
        "dptr": current_values.get("dptr", ""),
