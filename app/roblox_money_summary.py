...
def build_roblox_money_markdown(cfg: Config, payload: RobloxMoneyReportPayload) -> str:
    ...
    if payload.project_revenues:
        ...
        for revenue in payload.project_revenues:
            lines.extend(
                [
                    f"- {revenue.project_name}",
                    f"  当日收入（{revenue.report_date}）: {_format_usd(revenue.daily_usd)}（{_format_robux(revenue.daily_robux)} Robux）",
                    (
                        f"  月累计（{revenue.month_start_date} 至 {revenue.month_end_date}）: "
                        f"{_format_usd(revenue.month_to_date_usd)}"
    ...
    if payload.failures:
        ...
        for failure in payload.failures:
            ...
            lines.append(f"- {project_label}: {failure.reason}")
