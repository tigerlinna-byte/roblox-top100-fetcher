...
项目日报当前只写入一个名为“报错率”的稳定性字段，但实际数据来源是 Roblox Creator Analytics 的 Client crash rate，中文表头不够准确，也缺少帧率和服务器稳定性指标。运营需要在同一张按日期排列的日报表中查看每个日期对应的客户端与服务器性能数据，并能补齐历史日期。
