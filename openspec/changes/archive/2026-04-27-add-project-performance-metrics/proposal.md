...
项目日报当前只写入一个名为“报错率”的稳定性字段，但实际数据来源是 Roblox Creator Analytics 的 Client crash rate，中文表头不够准确，也缺少按设备拆分的客户端内存占比、帧率和服务器稳定性指标。运营需要在同一张按日期排列的日报表中查看每个日期对应的客户端与服务器性能数据，并能补齐历史日期。

## What Changes

- 将现有“报错率”表头改为“崩溃率”，继续使用当前 `client_crash_rate` / `ClientCrashRate15m` 数据源，不新增重复的客户端崩溃率列。
- 在“崩溃率”后、“更新时间”前新增以下项目日报列：
  - 平板内存，对应 Client Memory Usage Percentage by Tablet。
  - PC内存，对应 Client Memory Usage Percentage by Computer。
  - 手机内存，对应 Client Memory Usage Percentage by Phone。
  - 客户端帧率，对应 Client frame rate。
  - 服务器崩溃数，对应 Server crashes。
  - 服务器帧率，对应 Server frame rate。
- 不新增通用“客户端内存”或“服务器内存”列。
